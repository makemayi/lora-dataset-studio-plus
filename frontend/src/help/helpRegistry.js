...
const TOPICS = [
  ...
  action('bank-build-dataset', 'Build a dataset from a bank in one click',
    ['build', 'build dataset', 'one button dataset', 'dataset from bank',
     'one click dataset', 'face stills', 'waist-up', 'full-body', 'total images',
     'split', 'how many images', 'upscale', 'too small', '1536',
     'used more than once', 'reuse', 'reuse a picture', 'shortfall',
     'plan', 'build plan', 'what will this build', 'promote'],
    '/bank', 'using-the-app', 'build-a-dataset-from-a-bank-in-one-click'),
  ...
  setting('seedvr2.files', 'engines', 'seedvr2-engine', 'SeedVR2 model files (DiT + VAE)',
    ['seedvr2', 'vae', 'ema_vae_fp16', 'vae not found', 'dit', 'diffusion_models',
     '7b sharp', 'int8', 'convrot', 'model location', 'weights folder', 'which model',
     'share', 'same file as improve', 'klein improve']),
  setting('seedvr2.resolution', 'engines', 'seedvr2-resolution', 'SeedVR2 upscale multiplier',
    ['seedvr2', 'resolution', 'multiplier', '2x', '2 x', 'double', 'upscale size',
     'how big', 'enlarge', 'enlarged', 'short edge', 'vram', 'time', 'slower']),
  setupStep('setup-seedvr2-install', 'install', 'Install the SeedVR2 upscaler',
    ['seedvr2', 'seed vr2', 'seedvr', 'upscale', 'upscaler', 'upscaling', 'super resolution',
     ...
     'ComfyUI-SeedVR2_VideoUpscaler', 'comfyui-manager', 'dit', 'vae',
     '3b', '7b', 'fp8', 'install seedvr2']),
  ...
  action('video-collector-run', 'Run a local video collector',
    ['collector', 'local collector', 'video collector', 'run a collector',
     'douyin', 'kuaishou', 'tiktok', 'download videos', 'download an account',
     'save videos from', 'walk an account', 'rushes from scratch',
     'video\_collectors', 'nothing to run here', 'no collector is configured',
     'collector takes minutes', 'logged-in download'],
    '/video-bank', 'using-the-app', 'the-video-bank-turn-a-folder-of-rushes-into-shots'),
  ...
  { id: 'page-canvas', kind: 'page', title: 'LoRA Canvas',
    ...
    app: { route: '/canvas' } },
  { id: 'page-tasks', kind: 'page', title: 'Task Center',
    keywords: ['tasks', 'task center', 'queue', 'paused', 'waiting for comfyui', 'comfyui offline',
      'retry', 'job failed', 'cancel task', 'what is running', 'all jobs', 'task list',
      'which image', 'red dot', 'badge', 'captioning progress', 'stop captioning',
      'bank scan progress', 'what is my bank doing', 'dataset pass', 'bank pass',
      'stop a pass', 'how long left', 'eta', 'backup progress', 'watermark progress'],
    guide: { chapter: 'using-the-app', anchor: 'task-center-every-job-on-one-screen' },
    app: { route: '/tasks' } },
  ...
  setting('krea.grounding_px_by_framing', 'engines', 'krea-grounding-by-framing',
    'Krea 2 Edit reference grounding, per framing',
    ['krea', 'krea 2', 'grounding', 'grounding_px', 'per framing', 'face', 'bust', 'body',
     'back', 'framing', 'consistency', 'likeness', 'local engine']),
  setting('krea.ref_boost_by_framing', 'engines', 'krea-ref-boost-by-framing',
    'Krea 2 Edit reference boost, per framing',
    ['krea', 'krea 2', 'ref_boost', 'reference boost', 'per framing', 'face', 'bust', 'body',
     'back', 'framing', 'consistency', 'likeness', 'local engine']),
  ...
  setting('minimax_h3.vae', 'engines', 'h3-vae',
    'MiniMax H3 VAE file (both lanes)',
    ['minimax', 'minimax h3', 'h3', 'vae', 't1 image vae', 'video vae',
     'minimax_h3_t1_image_vae_step1597', 'decode', 'stills', 'both lanes',
     'local engine']),
  setting('minimax_h3.loras', 'engines', 'minimax-h3-loras',
    'MiniMax H3 generation — extra LoRAs',
    ['minimax', 'minimax h3', 'h3', 'generation', 'lora', 'loras', 'accelerator',
     'distill', 'turbo', 'lightning', 'chain', 'strength', 'local engine']),
  setting('krea.character_loras', 'engines', 'krea-character-loras',
    'Krea 2 Edit character LoRAs (chain of up to 5)',
    ['krea', 'character lora', 'character', 'person lora', 'second lora', 'two loras',
     'five loras', 'chain', 'ai-toolkit', 'trained lora', 'consistency', 'likeness',
     'strength', 'weight', 'local engine']),
  setting('krea.two_stage', 'engines', 'krea-two-stage', 'Krea 2 Edit two-stage sampler',
    ['krea', 'two stage', 'two-stage', 'dual resolution', 'KreaTwoStageSampler',
     'KreaDualResolutionSelector', 'stage2', 'accelerator lora', 'turbo lora',
     'krea2_turbo_lora_rank_64', 'handoff', 'upscale', 'local engine', 'advanced']),
  ...
  setting('klein.face_swap_loras', 'engines', 'klein-face-swap-loras', 'Face swap LoRAs',
    ['face swap', 'faceswap', 'swap', 'lora', 'loras', 'chain', 'klein',
     'strength', 'add lora']),
  setting('face_swap.engine', 'engines', 'face-swap-engine', 'Face / head swap engine',
    ['face swap', 'faceswap', 'head swap', 'headswap', 'swap engine', 'klein',
     'minimax', 'minimax h3', 'h3', 'which engine', 'change engine', 'slow swap']),
  setting('face_swap.h3_context_factor', 'engines', 'face-swap-h3-context',
    'MiniMax H3 swap — how much of the shot it sees',
    ['face swap', 'head swap', 'crop', 'cropped', 'zoom', 'context', 'framing',
     'head too big', 'head too small', 'proportion', 'shoulders', 'full body',
     'portrait', 'h3', 'minimax']),
  setting('face_swap.h3_mask_source', 'engines', 'face-swap-h3-mask-source',
    'MiniMax H3 swap — where the head mask comes from',
    ['face swap', 'head swap', 'mask', 'sam3', 'sam 3', 'automatic mask',
     'auto mask', 'segmentation', 'layerstyle', 'personmaskultra', 'region',
     'what to mask', 'h3', 'minimax']),
  setting('face_swap.h3_blend_pixels', 'engines', 'face-swap-h3-blend',
    'MiniMax H3 swap — how far the head is blended back',
    ['face swap', 'head swap', 'blend', 'seam', 'edge', 'join', 'feather',
     'pasted', 'does not blend', 'halo', 'h3', 'minimax']),
  setting('face_swap.h3_mask_opacity', 'engines', 'face-swap-h3-mask-opacity',
    'MiniMax H3 swap — how solidly the head is painted out',
    ['face swap', 'head swap', 'white face', 'blank face', 'grey face', 'empty face',
     'mask', 'opacity', 'placeholder', 'failed swap', 'h3', 'minimax', 'lama']),
  setting('face_swap.h3_stages', 'engines', 'face-swap-h3-stages',
    'MiniMax H3 (old) swap — optional stages',
    ['face swap', 'head swap', 'h3', 'minimax', 'stages', 'hair removal', 'lama',
     'inpaint', 'face detail', 'detailer', 'z-image', 'zimage', 'eyes', 'mouth',
     'bypass']),
  setting('face_swap.h3_new_stages', 'engines', 'face-swap-h3-new-stages',
    'MiniMax H3 (new) swap — optional stage',
    ['face swap', 'head swap', 'h3', 'minimax', 'new', 'stages', 'ollama',
     'describe', 'vision model', 'bypass', 'depth map']),
  setting('minimax_h3.swap_loras', 'engines', 'minimax-h3-swap-loras',
    'MiniMax H3 (new) swap — extra LoRAs on the swap model',
    ['minimax', 'h3', 'lora', 'loras', 'accelerator', 'speed', 'faster', 'slow',
     'distill', 'turbo', 'lightning', 'head swap', 'face swap', 'chain']),
  setting('minimax_h3.swap_steps', 'engines', 'h3-swap-steps',
    'MiniMax H3 (new) swap — sampler steps',
    ['minimax', 'h3', 'steps', 'sampler', 'speed', 'slow', 'faster', 'distill',
     'accelerator lora', '8 steps', '4 steps', 'head swap', 'quality']),
  setting('face_swap.h3_head_removal_prompt', 'engines', 'face-swap-h3-head-removal',
    'MiniMax H3 (new) swap — what the Klein pass is told to do',
    ['face swap', 'head swap', 'h3', 'minimax', 'klein', 'prompt', 'instruction',
     'mannequin', 'depth map', 'erase', 'remove head', 'big head', 'proportion',
     'head too large', 'doll', 'scale', 'negative prompt']),
  setting('face_swap.h3_new_ollama_model', 'engines', 'face-swap-h3-new-ollama-model',
    'MiniMax H3 (new) swap — which Ollama model analyses the head',
    ['face swap', 'head swap', 'h3', 'minimax', 'new', 'ollama', 'model', 'tag',
     'vision model', 'pulled', 'qwen3-vl', 'describe', 'head analysis', 'dropdown']),
  setting('face_swap.h3_ref_image_size', 'engines', 'face-swap-h3-ref-size',
    'MiniMax H3 swap — reference pipeline (max / match)',
    ['face swap', 'head swap', 'h3', 'minimax', 'reference', 'max', 'match',
     'likeness', 'resemblance', 'does not look like', 'identity', 'fidelity',
     'ref image size', 'ref_image_size', 'pipeline', '2048']),
  setting('engines.nanobanana_base_url', 'engines', 'engines-nanobanana_base_url', 'Nano Banana (Gemini) Base URL',
    ['nano banana', 'nanobanana', 'gemini', 'base url', 'endpoint', 'gateway', 'proxy',
     'reseller', 'compatible', 'cheaper', 'price', 'cost', '401', '404', 'privacy',
     'GEMINI_BASE_URL']),
