        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
          ...
          {state.status === 'error' && (
            <div className="mx-auto flex min-h-64 max-w-lg flex-col items-center justify-center text-center">
              <p role="alert" className="m-0 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">
                {state.error}
              </p>
          ...
          {state.status === 'ready' && state.limitMessage && (
            <p role="status" className="mx-auto mb-3 max-w-4xl rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[0.6875rem] text-amber-700">
          ...
          {state.status === 'ready' && selectedSeries && (
            <div className="mx-auto max-w-4xl">
              ...
              {selectedSeries.truncated && (
                <p role="status" className="m-0 mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[0.6875rem] text-amber-700">
              ...
              {frames.length === 0 ? (
                ...
                <>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 border-t border-border pt-3">
                    <div className="flex items-center gap-1" aria-label="Playback controls">
                      ...
                      <button type="button" onClick={() => setPlaying((value) => !value)}
                        ...
                        className="min-w-20 rounded-md border border-indigo-400/60 bg-indigo-50 px-3 py-2 text-[0.75rem] font-semibold text-indigo-700 hover:bg-indigo-500/25">
                        {playing ? '❚❚ Pause' : '▶ Play'}
                      </button>
                  ...
                  {exportState.error && (
                    <p role="alert" className="m-0 mt-2 rounded-md border border-rose-400/40 bg-rose-50 px-2 py-1.5 text-right text-[0.6875rem] text-rose-700">
                      {exportState.error}
                    </p>
                  )}
                </>
