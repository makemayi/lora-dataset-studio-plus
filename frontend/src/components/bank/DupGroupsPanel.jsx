          ...
          <li key={g.group} className="rounded-lg bg-surface p-3">
            <div className="mb-2 flex items-center gap-2 text-xs text-content-muted">
              ...
              <button type="button" disabled={busy || live}
                ...
                className={`rounded-full bg-surface-raised px-2.5 py-0.5 text-content ${FLOAT_SHADOW} transition-[box-shadow,transform,background-color] duration-200 hover:bg-surface hover:-translate-y-0.5 disabled:opacity-50`}>
                Keep best
              </button>
              <button type="button" disabled={busy || live}
                ...
                <li key={img.id} className="w-32">
                  <button type="button" disabled={busy || live}
                    ...
                    <img src={`/api/bank/${bankId}/thumb/${img.id}`} alt={img.name}
                      loading="lazy" className="h-24 w-full object-cover object-top" />
                    {img.id === g.best_id && (
