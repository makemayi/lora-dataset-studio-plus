...
export default function BankWatermarkMaskDialog({ bankId, image, onSaved, onClose }) {
  ...
  const btn = `min-h-11 rounded-lg ${FLOAT_SHADOW} border border-white/20 bg-white/10 px-3 text-xs `
      ...
      <div className="shrink-0 space-y-2 border-t border-white/10 bg-black/70 px-3 py-2.5">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          <button type="button" aria-pressed={addMode} onClick={() => setAddMode((v) => !v)}
            ...
            className={addMode
              ? `min-h-11 rounded-lg ${FLOAT_SHADOW} border border-sky-300 bg-sky-500/25 px-3 text-xs font-semibold text-sky-100`
              : btn}>
            + Add zone
          </button>
