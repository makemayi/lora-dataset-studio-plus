      ...
      <div className="flex max-h-[calc(100vh-1.5rem)] w-full max-w-lg flex-col gap-3 overflow-y-auto rounded-2xl bg-surface-overlay/85 backdrop-blur-md p-4 shadow-2xl sm:max-h-[calc(100vh-2rem)]">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={onClose} aria-label="Close dataset picker"
            className={`h-8 w-8 shrink-0 rounded-full bg-surface-raised text-content-muted ${FLOAT_SHADOW} transition-[box-shadow,transform,background-color] duration-200 hover:bg-surface hover:-translate-y-0.5 hover:text-content`}>
            ×
          </button>
        </div>

        {loading && (
          <p className={`m-0 flex items-center gap-2 rounded-lg bg-surface-raised ${FLOAT_SHADOW} px-3 py-2 text-[0.75rem] text-content-muted`} role="status">
        ...
        {error && (
          <div className={`rounded-lg ${FLOAT_SHADOW} border border-red-400/40 bg-red-50 px-3 py-2 text-[0.75rem] text-red-700`} role="alert">
        ...
        {!loading && !error && datasets.length === 0 && (
          <p className={`m-0 rounded-lg ${FLOAT_SHADOW} border border-amber-200 bg-amber-50 px-3 py-2 text-[0.75rem] text-amber-700`} role="status">
            ...
            {datasets.map((dataset) => {
              ...
              return (
                <button type="button" key={choice.id} onClick={() => onChoose(choice)}
                  className={`flex min-w-0 items-center gap-3 rounded-xl bg-surface-raised ${FLOAT_SHADOW} px-3 py-2.5 text-left hover:border-purple-400/60 hover:bg-purple-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-400`}>
                  ...
                  <span className="shrink-0 text-[0.6875rem] font-semibold text-purple-700">Use &amp; draw</span>
                </button>
              );
            })}
          </div>
...
export default function DatasetCaptionControl({ onCaption }) {
  ...
  return (
    <>
      ...
      {lockedDataset && (
        <span className="inline-flex max-w-full items-center gap-1 rounded border border-purple-400/30 bg-purple-50 px-1.5 py-0.5 text-[0.625rem] text-purple-700"
      ...
      {error && (
        <span className={`basis-full rounded-lg ${FLOAT_SHADOW} border border-red-400/40 bg-red-50 px-2 py-1.5 text-[0.6875rem] leading-snug text-red-700`} role="alert">
          {error.message}{' '}
