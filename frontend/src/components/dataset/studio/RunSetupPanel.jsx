  ...
  return (
    <>
      ...
      {d.gpu_busy && !d.comfyui_recovery?.requires_comfyui_restart_confirmation && (
        <div className={`m-0 flex flex-wrap items-center gap-2 rounded-lg ${FLOAT_SHADOW} border border-red-400/40 bg-red-50 px-3 py-2 text-red-600 text-sm`} role="status">
          ...
          {d.comfyui_recovery_target?.dataset_id != null && (
            <button type="button"
              ...
              className={`ml-auto rounded-lg ${FLOAT_SHADOW} border border-red-300/40 bg-red-400/15 px-2.5 py-1 text-xs font-semibold text-red-100`}>
              Open paused test →
            </button>
          )}
        </div>
      ...
      {d.comfyui_recovery?.requires_comfyui_restart_confirmation && (
        <div className={`flex items-center gap-2 flex-wrap rounded-lg ${FLOAT_SHADOW} border border-amber-200 bg-amber-400/10 px-3 py-2`} role="status">
      ...
      {/* --- Run en cours ------------------------------------------------ */}
      {d.pending > 0 && (
        <div className={`flex items-center gap-2 rounded-lg ${FLOAT_SHADOW} border border-indigo-200 bg-indigo-50 px-3 py-2`} role="status">
      ...
      {!d.pending && d.resumable > 0 && (
        <div className={`flex items-center gap-2 rounded-lg ${FLOAT_SHADOW} border border-amber-200 bg-amber-400/10 px-3 py-2`} role="status">
          <span aria-hidden>⏸</span>
      ...
      {!d.pending && (
        <div id="st-setup" className="flex flex-col gap-2 scroll-mt-16">
          ...
          {launchHint && (
            <p className={'m-0 text-[0.6875rem] ' + (launchBlocked ? 'text-amber-700' : 'text-content-muted')}
