...
import {
  CARD_SURFACE, INPUT_CLASS, PRIMARY_BUTTON, QUIET_BUTTON,
} from '../components/common/surfaces'

/* The wizard shares the app's surfaces since 2026-08-10 — its own copy of the
   input class was the same string, and its cards were outlined boxes while
   Settings' were shaded. `max-w-xl` comes with the shared one: the wizard is
   `max-w-2xl` anyway, so a field is bounded the same way it is in Settings. */
const PRIMARY_ACTION = PRIMARY_BUTTON
const SECONDARY_ACTION = QUIET_BUTTON
...
export default function SetupPage() {
  ...
  // One cheap readiness read on entering the wizard, from a dedicated one-second
  // endpoint instead of the expensive capability scan. It no longer polls: with
  // no bundled runtime left, nothing here comes up on its own after the page
  // loads. Unmount aborts the active fetch and clears any pending timer.
  useEffect(() => {
    let alive = true
    let timer = null
    let controller = null
    ...
    return loadError ? (
      <div className="space-y-3">
        ...
        <button type="button" onClick={load}
          className={`${SECONDARY_ACTION} text-sm`}>
          Retry
        </button>
      </div>
    ) : (
  ...
  const saveRecheckBtn = (
    <button type="button" onClick={persist} disabled={busy}
      className={`mt-1 ${SECONDARY_ACTION}`}>
      {/* Both labels mounted — see CLAUDE.md ▸ UI changes. */}
      <span hidden={!busy}>Saving…</span>
      <span hidden={!!busy}>Save &amp; re-check</span>
    </button>
  )
  ...
  const toolBody = (id) => {
    ...
    if (id === 'comfyui') {
              ...
              <div className="space-y-1.5">
                ...
                {v.suggestion && (
                  <button type="button" onClick={() => setField('comfyui', 'base_dir', v.suggestion)}
                    className={`${SECONDARY_ACTION} text-primary`}>
      ...
      const startComfyuiNode = comfyLauncher.visible && (
        <div className="space-y-1.5">
          <button type="button" onClick={startComfyui}
            ...
            className={`${SECONDARY_ACTION} text-sm disabled:cursor-not-allowed`}>
            <span hidden={!startingComfyui}>Starting…</span>
            <span hidden={!!startingComfyui}>▶ Start ComfyUI</span>
          </button>
        ...
        <>
          ...
          {step.reachable && !step.hasKlein && (
            <div className="space-y-1 text-xs text-content-muted">
              ...
              {step.dirValid ? (
                <div className="space-y-2 rounded-lg bg-surface-raised p-2.5">
      ...
      const skipPanel = (
        <div className="space-y-4">
          <div className="space-y-3 rounded-lg bg-surface-raised px-4 py-3 text-sm">
            ...
            <div className="flex items-center gap-4 pt-1">
              <button type="button" onClick={skipComfyui} disabled={busy}
                className={`${PRIMARY_ACTION} text-xs`}>
                <span hidden={!busy}>Saving…</span>
            <span hidden={!!busy}>Continue without ComfyUI</span>
              </button>
      ...
      if (step.skipped) {
        return (
          <div className="space-y-4">
            <div className="rounded-lg bg-surface-raised px-3 py-2 text-sm text-content-muted">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {ML_CAPS.map((c) => {
              ...
              return (
                <div key={c.action} className="rounded-lg bg-surface-raised p-3 space-y-2">
          ...
          <details className="rounded-lg bg-surface-raised px-3 py-2">
          ...
          <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-3 text-sm text-content space-y-2">
            ...
            {verdict.candidates.map((p) => (
              <button key={p} type="button"
                ...
                className={`w-full text-left text-xs sm:w-auto ${PRIMARY_ACTION}`}>
  ...
  const nextWithSave = async () => {
    ...
    if (kind === 'comfyui') {
      ...
      if (!cfgDir && !s.reachable && !s.skipped) {
  ...
  if (kind === 'welcome') {
    ...
    const scanRows = [
      ...
      { label: 'Captioning — Ollama + vision model', optional: true, stepId: 'ollama',
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <section className={`p-5 ${CARD_SURFACE}`}>
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-content">
              <span hidden={!detecting}>Scanning your machine…</span>
              <span hidden={!!detecting}>Machine scan</span>
            </h2>
            {detecting
        ...
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={goNext}
            ...
            <span hidden={!allReady}>Everything&rsquo;s ready — review →</span>
            <span hidden={!!allReady}>Start setup →</span>
          </button>
        </div>
      </div>
    )
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/datasets" className={PRIMARY_ACTION}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button type="button" onClick={nextWithSave} disabled={advancing}
            title={reason || ''}
            className={`${PRIMARY_ACTION} disabled:cursor-not-allowed`}>
            {nextLabel}
          </button>
        </div>
      </div>
