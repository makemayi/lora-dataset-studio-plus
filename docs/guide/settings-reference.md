...
- **Character LoRA (optional)** → `krea.character_lora`. A SECOND LoRA — your own trained character/person LoRA (e.g. from ai-toolkit) — chained after the identity edit LoRA above for extra likeness on top of Krea's baseline consistency. Blank (default) = off, the graph is unchanged from before this setting existed. Unlike the identity edit LoRA there is no auto-detection: this names one specific file you trained, so a typo surfaces as ComfyUI's own "file not found" rather than silently using a guess.
- **Character LoRA strength** → `krea.character_lora_strength`. Default **`0.8`**, range 0–1.5. Only shown once a character LoRA is set. Higher locks the character harder but can fight the edit prompt (pose/outfit/scene changes stop landing); lower lets the edit through more but weakens the likeness boost.
