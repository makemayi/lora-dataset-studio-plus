...
| `engines.chatgpt_comfy_quality` | `low`/`medium`/`high` (default `medium`) asked of the ComfyUI lane's node. The one dial that moves what a picture costs, in credits and in waiting; ignored by the two direct lanes. |
