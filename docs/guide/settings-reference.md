...
**Image quality on this lane** → `engines.chatgpt_comfy_quality` (`low` / `medium` / `high`, default **`low`**). The only dial here that moves what a picture costs — in credits *and* in waiting. The wait is OpenAI's, not your GPU's: three shots at `high` took **1'55", 2'11" and 2'12"** on the maintainer's machine, whether they came back with an image or a refusal. Ignored by the two direct lanes. The catalog card's own ratio picks the node's `size`; a ratio the node has no size for asks it for `auto`.

**How long it waits.** 15 minutes per shot, then the row fails. Every reference photo is uploaded and processed by the provider, so the shots carrying the most references are the slow ones — measured on the maintainer's install: one reference at `low` came back in 47 s, a four-reference shot was still running at 4 minutes. When the wait ends, a shot still QUEUED in ComfyUI is removed from the queue (an exact-id delete, never the global interrupt that would kill another render); one already RUNNING cannot be stopped that way, and the message says so rather than implying the credits were saved.

**Which node it drives.** `OpenAIGPTImageNodeV2` when your ComfyUI has it, `OpenAIGPTImage1` otherwise. V2 is preferred for one reason that is not cosmetic: it takes the references as **16 separate slots**, the shape gpt-image's edit endpoint actually has and the same 16 the direct API lane allows, while V1 has a single image input that several references have to be flattened into first.
...
- **Reference grounding, per framing** → `krea.grounding_px_by_framing.{face,bust,body,back}`. Blank (default) for a framing = use the dial above for it — nothing changes until you touch this. The same pixel budget is not equally strong on every framing: on a face close-up the reference IS almost entirely face, so identity is already solid and a LOWER value there buys more expression/angle/background variety without losing it; on a full-body shot the face is a small fraction of the same reference, so a HIGHER value can help identity hold up where it is most likely to drift.
- **Reference boost** → `krea.ref_boost`. Range `0`–`10`, default **`4`**. How hard the source latent is pushed back into the model each step — the same consistency ↔ prompt dial as grounding above, at a different pipeline stage. **Higher** = stronger identity retention, less room for the prompt to change pose/outfit/scene; **lower** = more freedom, weaker likeness.
- **Reference boost, per framing** → `krea.ref_boost_by_framing.{face,bust,body,back}`. Shipped non-blank (`face 4`, `bust 6`, `body 8`, `back 8`) — same rationale as grounding per framing: a face close-up already holds identity at full strength, but a bust/body/back shot dilutes that signal across more of the frame, so a HIGHER boost is what it takes to hold the same identity there.
...
- **Two-stage sampler (advanced)** → `krea.two_stage`. Off by default. Swaps the single sampler above for `KreaTwoStageSampler` + `KreaDualResolutionSelector`: a low-resolution first pass handed off to an upscaled second pass, plus a stage-2-only accelerator LoRA (`krea2_turbo_lora_rank_64_bf16.safetensors`, found by a narrow scan of your loras folders — it is **not auto-installed**, unlike the four required assets above). Needs a newer build of the comfyui-krea2edit node pack that exposes both classes; the engine card names them if absent. CFG and the sampler/scheduler choices are fixed to the calibrated export this mirrors, not Settings dials — **Sampler steps** above only applies to the single-stage path. **Forces a Raw base model** — MEASURED: the Turbo build does not hold up through the stage1/stage2 handoff, so this path ignores Turbo entirely, including an explicit **Base model file** pinned to one, and resolves a Raw build instead; a Turbo-only install reports the base model as missing while this is on.
  - **Stage 1 steps** → `krea.two_stage_stage1_steps` (default `52`, 2–150). **Stage 2 steps** → `krea.two_stage_stage2_steps` (default `12`, 2–150). Lower either to trade quality for speed — the calibrated export's 52+12 total is deliberately high-quality-first, not fast.
  - **Handoff (%)** → `krea.two_stage_handoff_percent` (default `16.67`, 0–100). Where stage 1 hands off to stage 2 in the denoising schedule — `0` uses only stage 2, `100` uses only stage 1.
  - **Stage 1 megapixels** → `krea.two_stage_base_megapixels` (default `1.0`, 0.1–16.0). The low-resolution first pass's target size.
  - **Final megapixels** → `krea.max_output_mp` (default `2.0`, 0.1–16.0). The two-stage path's final upscaled size — **and** the single-stage sampler's output-size cap above, since both share the same sizing call. Lowering it speeds up either path.
...
- **Optional accelerators** → `minimax_h3.use_speed_nodes`,
  ...
  install that lacks them. The speed nodes (attention backend + memory-efficient
  Sage attention patch) do not change the image. The RTX 2× upscale is NVIDIA
  RTX only — without it you lose the upscale, not the engine.
- **Ref2VA model file** → `minimax_h3.base_model`. Blank (default) finds any
  Ref2VA build on its own — int8, int4 or mixed. A filename that is no longer on
  disk falls back to auto rather than failing. Since 2026-08-16 the graph also
  takes the `fl2va` sibling when it is present: the generation lane then runs
  the maintainer's hybrid loader (Ref2VA laid over Fl2VA's last twenty blocks),
  and falls back to a plain Ref2VA load when it is not — Fl2VA is a 66 GB
  download, and no install is forced to fetch it.
- **VAE file** → `minimax_h3.vae`. Blank (default) resolves in one order: the
  T1 **image** VAE (`minimax_h3_t1_image_vae_step1597.safetensors`) when it is
  on disk, else the video VAE. **One key for both H3 lanes** — generation and
  the new head swap decode through the same VAE, so the two can never disagree.
  The image VAE is the one the maintainer's 2026-08-16 graphs decode with, and
  both lanes sample a one-frame packet then decode a still, where a VAE trained
  for stills is visibly cleaner on skin and hair than the video one. A filename
  that is no longer on disk falls back to the auto order rather than failing.
- **Extra LoRAs on generation** → `minimax_h3.loras` (up to 4 rows of
  `{file, strength}`), the generation twin of the swap's list. It exists for
  the same reason: no graph here can ship one, because the maintainer's export
  loads a 4-step turbo distill and a realism LoRA by name from a folder that
  exists on one disk. Rows chain onto the model after the optional speed
  patches; a row whose file is not on disk is skipped with a server-log line,
  not failed. A step-distill LoRA here means lowering **Sampler steps** above to
  the number its model card names.
...
- **Nothing is auto-downloaded.** Klein and Krea can be installed from Setup;
  ...
  is likewise a manual install (then restart ComfyUI). One file is conditional:
  the CLIP-ViT-H vision tower only enters the job when **Frames per shot** is
  above 1 — at the default single frame the still is taken by a core node, so an
  install without the vision tower generates fine until a packet is sampled.
...
- **two model files, in the folders the improve lane already uses** (this engine runs the same manual pipeline the ✨ improve 'klein' lane's SeedVR2 half runs, so one install serves both):
  - the DiT — `seedvr2_7b_sharp_int8_convrot.safetensors` (8.3 GB) in `models/diffusion_models`. This is a community INT8 re-quantisation with **no public URL**; if the improve lane works on your install, this file is already there. The app names its exact path when it is missing.
  - the VAE — `ema_vae_fp16.safetensors` (0.5 GB) in `models/vae`, from [numz/SeedVR2_comfyUI](https://huggingface.co/numz/SeedVR2_comfyUI) (Apache-2.0, public). The Setup card downloads this one on a click; an `extra_model_paths.yaml` root works too.
...
**Large frames are tiled by the CORE tiled VAE, always.** The pipeline encodes and
decodes in 512px tiles (128px overlap, temporal window 4096), so a 4K source never
has to fit the card in one piece — there is no separate tiling pack, no lane
choice and no VRAM ceiling to warn about. The former TTP-based high-resolution
lane and its settings (`seedvr2.tiling` / `tile_px` / `tile_threshold`) were
removed with the 2026-08-16 rebuild.

**There is no batch-size setting, on purpose.** SeedVR2's batch size is a *video*
window whose frames share attention to stay coherent — feeding it unrelated
photos would let them bleed into each other. Images are upscaled one per job; the
throughput comes from the normal generation queue.

**The tile toolbar button is the destructive route, and it says so.** ✨
Upscale & improve creates a candidate and never touches the source; the 🔍 2x
button on a tile and in the lightbox sends that image through SeedVR2 and
REPLACES the tile in place. The original is kept for ↩ Undo and restored if the
run fails — the same contract as the face swap — but it is still an overwrite,
so the button's tooltip names it: *"replaces the tile, original kept for undo"*.
...
No launcher downloads a model: use the explicit **Pull** button in LDS to see progress and cancel or resume the transfer.
...
- **Training method** → `onetrainer.peft_type`. Default **`LORA`**, the adapter this app's own Krea 2 Edit inference graph loads via `LoraLoaderModelOnly`. **`OFT_2`** (Orthogonal Finetuning) is a different adapter algorithm, offered as an alternative — user-confirmed to train and load correctly, not a recommended default swap. An unrecognised value (a hand-edited config, an older save) degrades to `LORA` at launch time rather than failing the run. This also fixed a real bug: `lora_rank` was already app-owned and user-configurable, but `lora_alpha` and `batch_size` were left to the shipped preset's own defaults — a rank change without a matching alpha silently rescales a LoRA's effective strength (alpha/rank), and a mismatched batch_size changes how many optimizer steps the epoch count you asked for actually buys. `lora_alpha` is now pinned to equal the chosen `lora_rank` (scale 1.0) so every launch trains at its intended strength, while `batch_size` comes from the OneTrainer batch field on the panel — and, when that field is empty, from this lane's own default of **4** at **512 px** (2026-08-29). Those two travel together: 512 is the shipped preset's own resolution, and it is what leaves a 24 GB card room for a batch of 4. The lane trained at 1024 before, which filled a 3090 to 98.5% at the preset's batch of 2. An explicit resolution in Advanced options still wins over the default. **Preview prompts and cadence reach this lane too** (since 2026-08-29): each run writes its own `samples.json` beside its config and points OneTrainer at it — before that the config pointed at the empty list shipped inside the OneTrainer install, so a run that reported a sample cadence produced no images at all. Previews render at the run's training resolution rather than Krea 2's 1024 sample default, because they are rendered with the training weights resident.
...
- **Subject similarity minimum** → `bank.subject_sim_min`. Cosine similarity to the photo picked with the **🎯 set-as-subject** tile button below which an image is flagged **🎯 not the subject** — the 🧹 Auto-reject pile for wrong-person cleanup. Default **`0.45`**. Needs 🎯 set-as-subject first (a tile button on any face the 👥 pass measured; it scores every cached embedding against that one face — no model run). A convincing lookalike can clear the floor, so glance at the pile before confirming. *Applies immediately* — the bank re-sorts, no re-run.
...
- **Video bank sources** → `paths.video_sources_dir`. Where the app puts the folder of a video bank that was created **by name alone** (the folder field left empty): one subfolder per bank, named after it and suffixed `-2`, `-3`… on a name clash. These start empty — filling one is what a collector run is for. Default **empty → `<data dir>/video_sources`**. This root, unlike the four above, has no **Move** surface: it only ever holds folders the app itself created, and a bank can be relocated from its own page.
...
| `paths.video_sources_dir` | Where a video bank created **by name alone** gets its (initially empty) source folder: one subfolder per bank, suffixed `-2`, `-3`… on a name clash. Empty string defaults to `<data dir>/video_sources`. |
...
| `ollama.deployment_mode` | Dead key. It picked an Ollama endpoint for the Docker deployments, which were removed; nothing reads it any more. An existing `config.json` still loads with it — `ollama.url` is the setting that decides. |
...
| `engines.chatgpt_comfy_quality` | `low`/`medium`/`high` (default `low`) asked of the ComfyUI lane's node. The one dial that moves what a picture costs, in credits and in waiting; ignored by the two direct lanes. |
...
| `face_swap.h3_new_stages` | One boolean for the NEW H3 graph, `false` by default: `ollama` (an Ollama vision call describes how the head sits in this photo and that goes into the instruction — runs on `face_swap.h3_new_ollama_model`, and suppresses `h3_pose_hint` while on). The old `mask_overlay` entry left with the 2026-08-16 graph, which has no AILab_MaskOverlay node; a stored value is ignored, not rejected. |
...
| `face_swap.h3_mask_source` | Where the OLD H3 swap's head mask comes from (`minimax_h3_old` only — the new graph always masks in the app): `graph` (the workflow's own segmenter, PersonMaskUltra/ComfyUI_LayerStyle) or `app` (services/auto_mask — SAM 3 in the app's own environment, on the sam3.pt ComfyUI already has; one child at a time). Default `graph`. |
...
| `face_swap.h3_mask_opacity` | **`minimax_h3_old` only.** How opaque the placeholder painted over the head is (0–1, default `1.0`). Below 1.0 a ghost of the ORIGINAL head shows through, so 0.75 measurably returns the original face — keep 1.0 (and it is why the `lama` stage cannot change anything at 1.0). On the new engine it does nothing: the 2026-08-16 graph has no overlay node, and the head reaches H3 as the grey skull stand-in with nothing on top of it. |
...
| `minimax_h3.swap_steps` | **`minimax_h3` (new) only.** Sampler steps for the swap, `0` = the graph's own 8 (clamped 1–100). Paired with `swap_loras`: running a 4-step distill for 8 steps is slower than the stock model AND worse, and nothing reports it. |
...
- **Max output side** → `topaz.max_output_side` (default **2048**, 0 = uncapped). Topaz's Autopilot picks its own scale factor and measurably goes 4x+ on small sources — without this cap a 768×1024 tile comes back as a 65 MB 6000×8008 PNG that training crops back to 1024 anyway. Anything with a longer side is Lanczos-downscaled after the run; the CLI itself offers no working scale override (its numeric settings keys fail at the engine level while the process still exits 0 — measured on 4.0.1).

Topaz runs in **Autopilot** mode for model and scale: the app only turns the upscale enhancement on (plus the noise/sharpen/lighting/color toggles). Since 2026-09-12 it does NOT let Autopilot touch faces: Topaz's own Face Recovery (Autopilot enables it at strength 0.8 on every face) is what makes upscaled portraits look plastic. Instead the app detects each image's largest face (local CPU detection) and decides per image — faces ≈512px and larger run with face recovery **off**, smaller faces get a gentle strength (0.3–0.7) scaled to how small they are; images with no detectable face — and every image if the face-detection env is unavailable — run with it off too. Each image's result records the decision. A "Topaz is not logged in" failure means the desktop app has not been opened on this machine to complete the license sign-in once.
