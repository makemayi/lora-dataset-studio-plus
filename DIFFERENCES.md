...
## 🎞 MiniMax H3（第三个本地生图引擎，全新功能）

Klein、Krea 2 Edit 之外的第三个本地引擎，同样是"一张参考照 + 无需角色 LoRA"，
但它是个**视频模型**：每个镜头采一小段帧，再从中挑出最好的一帧当成品。

- 在 RTX 3090 上实测：换一张卡片约 **78 秒**，同一张卡片再出一张约 **38 秒** ——
  文本编码只取决于提示词，所以批量生成会把同一张卡片的多份**连续入队**，
  100 张图分布在 20 张卡片上是 75 分钟而不是 130 分钟
- 复用已有的左45/右45角度参考照：侧面镜头喂对应角度的照片
- 三个提速节点和 RTX 超分都是**可选**的：ComfyUI 有就接上，没有就跳过，出图一样
- 设置项：参考图尺寸（match/max，这是它的相似度旋钮）、每镜头帧数、挑帧的相似度权重、
  采样步数、参考图降采样边长、Ref2VA 模型文件

两条限制写在明处：

- **没有任何自动下载**。五个模型文件约 40 GB，外加 `MinimaxH3-Image` 节点包，
  全部手动放置 —— 引擎卡片会逐个点名缺什么、放哪里
- **ComfyUI 必须用 `--disable-dynamic-vram` 启动**。H3 的权重总量超过 24 GB 显卡的容量，
  不加这个参数不会报错，只是随机变慢：实测同样的活从 69 秒到 397 秒不等。
  应用在能读到启动参数时会给出警告
- 只做数据集生成，不做参考图编辑（✦ 编辑参考图仍然是 Klein / Krea / API 引擎）
