...
_HINT_PROMPT = (
    '这是同一个人的几张照片。用一句中文逗号短语（不超过50字）客观描述她的长相：'
    '脸型、眼睛、眉毛、鼻子和嘴唇、发型与发色、肤色、脸上的痣或疤痕、以及眼镜或耳饰。'
    '只写长相本身，不要提照片、光线、背景，也不要复述这段话。'
    '/no_think')
...
def _extract_phrases(text):
    ...
    for chunk in re.split(r'[。；;，,、\n]', text):
        ...
        # 剥掉与事实粘连的条目名前缀（'鼻唇鼻梁适中'→'鼻梁适中'）；只剥多字
        # 条目名——单字的（眉/眼/鼻/唇/痣）会把 '眉毛自然' 剥成 '毛自然'。
        for label in _GLUED_LABELS:
            if chunk.startswith(label) and len(chunk) > len(label) + 1:
                chunk = chunk[len(label):].strip(' ：:，,')
                break
        if chunk not in seen:
            seen.add(chunk)
            out.append(chunk)
    # 子串去重：'鼻头' 已在更长短语里出现就不重复列
    return [p for p in out if not any(p != q and p in q for q in out)]


# 只列会与事实粘连的多条目名；'肤色'/'脸型' 不在其中——'肤色白皙' 是合法
# 短语，剥成 '白皙' 反而丢信息（实测把测试都搞挂了）。
_GLUED_LABELS = ('发型与发色', '发型发色', '脸部特征点', '脸部饰品', '脸部特征',
                 '面部特征', '鼻唇')
