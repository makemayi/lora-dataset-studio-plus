...
- **🎭↔ Swap a tile's face with your reference photo, in one click — on any of three engines** — every tile with a current image gets a **🎭↔** button next to 🔄 Regenerate: the tile becomes the base shot, the dataset's reference photo supplies the identity, and the swap overwrites the tile in place. **Klein** (the default) repaints the head with a swap LoRA — one graph, fast, and it needs that LoRA on disk. **MiniMax H3 (new)** erases the head with a Klein pass, leaving a depth map of where it was, and lets the video model re-render the shot around your reference — no swap LoRA, and no seam to hide, because there is no join. **MiniMax H3 (old)** is the previous graph, kept on purpose: it masks the head, sends only a crop through H3 and composites it back. **The difference that decides between them is a limit, not a preference:** the new graph re-renders the **whole frame**, so the body, clothing and background come back re-drawn rather than preserved; the old one only ever repaints inside the mask. Both H3 graphs load the whole ~40 GB H3 stack and are much slower than Klein, and the new one needs the Klein models as well. Optional extras ship off on both — a blue mask overlay and an Ollama pass that reads the photo and describes how that head sits (new), Klein hair removal, LaMa cleanup and a Z-Image eye/mouth detailer (old). The engine is a **setting, not a per-click choice** (Settings → Image engines → Klein), each engine is still **one fixed workflow rather than a configurable pipeline**, and the button only appears once the dataset has a reference photo to swap in.
...
| MiniMax H3 generation | ComfyUI reachable **and launched with `--disable-dynamic-vram`** + the `MinimaxH3-Image` node pack + five model files (Ref2VA model, Qwen3-VL 32B encoder, video VAE, audio VAE, CLIP-ViT-H); none auto-downloaded; [exact files](docs/guide/settings-reference.md#minimax-h3-local) |
