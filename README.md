...
| **Topaz Photo AI upscaling** | The 🔍 upscale button can run Topaz Photo AI (own GPU, outside ComfyUI) instead of SeedVR2 — pick the engine in Settings. Topaz jobs queue behind the GPU and show in the Task Center with cancel/retry; results replace the tile with the original kept for undo |
