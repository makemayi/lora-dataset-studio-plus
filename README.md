...
Everything the app can do, in one long read: [getting started](docs/guide/getting-started.md) · [the full workflow](docs/guide/workflow.md) · [every setting explained](docs/guide/settings-reference.md) · [troubleshooting](docs/guide/troubleshooting.md).
...
| **Promote with framings** | Promoting into a dataset can emit three framings per kept picture — the full frame, a waist-up crop and a face close-up cut around the measured face box (the face fills at least 60 % of its picture). Crops inherit the picture's caption, are deduplicated against re-runs by provenance rather than by hash, and pictures without a usable face still promote their full frame |
...
| **Crop to person** | One pass crops every non-rejected image around its largest detected person (Grounding DINO): 3 % padding, the image's own aspect ratio preserved, so the person fills the frame without changing the dataset's shape. Pictures with no person are skipped, not padded; already-cleaned or manually turned images are skipped too, so no undo ever has to untangle two stacked crops. The crop is a working copy — sources stay untouched, and Undo removes it |
...
| **Extract frames into an image dataset, with quality gates** | The same kept clips can become an **image** training set instead of a video one. Three person modes (no requirement / frame must show a person — no reference needed / must be the reference person), a sharpness tolerance and a separate face-sharpness gate that catches a moving face inside a sharp frame. Three **framings** per pick — full frame, waist-up and a face close-up cropped to the measured face box, each taken from different moments — so a full-body frame's postage-stamp face never has to carry identity alone; identity mode also refuses frames holding a **second face**, and a deep head tilt. Everything rejected is reported per clip with its reason when the job finishes |
...
| **Topaz Photo AI upscaling** | The 🔍 upscale button can run Topaz Photo AI (own GPU, outside ComfyUI) instead of SeedVR2 — pick the engine in Settings. Topaz jobs queue behind the GPU and show in the Task Center with cancel/retry; results replace the tile with the original kept for undo. **Bulk:** select several images and 🔍 Upscale in place runs them as ONE batch — Topaz loads its models once instead of per image — with k/N progress and a retry that re-runs only the images that failed. **Skin stays natural:** Topaz's Face Recovery is what makes upscaled portraits plastic, so the app measures each image's largest face and only lets it work (gently, strength 0.3–0.7) on faces smaller than ≈512px — big sharp faces, no-face images and unavailable detection all run with it off |
| **Task Center** | Every background job in one live list with ComfyUI and GPU status — generation, training, vision, Topaz upscales, the long **dataset passes** (captioning, watermarks, face scoring, exports, backups, subject trim) and the image bank's own **scans, grouping and de-duplication**, each with its progress and, for bank passes, the time left. Submit even while ComfyUI is off: tasks pause and resume in order. **Cancel is only offered where a stop really exists** — a pass with no safe seam shows no button rather than one that does nothing, and a stop lands at the next image, keeping what was already written. Dataset and bank passes **cannot be retried from here** (they keep no recipe) and **do not survive a restart** — they live in memory, so a restart ends the work and the row together. The nav badge shows in-progress tasks and flags failures until you open the page |
...
| **Two local engines** | **ai-toolkit** (default) or **OneTrainer**. They do not read the same settings: OneTrainer trains by **epoch**, so it gets epochs, batch size and separate text-encoder rates, and most advanced levers now reach it too (effective batch, dropout, EMA, min-SNR, save/sample cadence, rank). Alpha still follows the chosen rank, and the network type stays fixed on that lane. It rides OneTrainer's own shipped Krea 2 recipe, with three exceptions the app owns even when the panel is empty: **512 px** and **batch 4** (the recipe's own resolution, and a batch chosen for a 24 GB card at that size), and an alpha pinned to the chosen rank. Everything else is that recipe until you set a value. Both lanes show live progress in the panel — OneTrainer reports the epoch it is on alongside the global step, since that is the unit it trains in. Both lanes also write preview images during the run, from the same editable prompt list, rendered at the size the run trains at. A finished OneTrainer run auto-deploys its checkpoint into the ComfyUI LoRA folder, so it shows up on the Canvas / LoRA Studio like any other checkpoint. **OneTrainer keeps no intermediate checkpoints** — its shipped recipe saves once, at the end, so the previews are all you have to judge a run by until it finishes. **Stopping it is not the same as killing it**: it is asked to stop, writes its backup and saves the LoRA it has trained so far, and holds the GPU until that is written (a kill is the fallback for a run that will not go) |
...
- **🆕 SeedVR2 — a second upscale engine that doesn't repaint your images** — the default ✨ Upscale & improve pass runs a small edit for sharpness/clarity, colour-matched back to your original, then SeedVR2 for the actual detail restoration — but it still re-renders that edit stage from a prompt. **SeedVR2** is the fully conservative alternative: resolves detail at a higher resolution, no edit pass, content untouched. Pick either one from the bulk actions on a selection, or set your default for the single-image pass in Settings ▸ Image engines — the node pack is added from ComfyUI and the VAE is downloaded from Setup; the DiT is the same 7B Sharp int8 build the improve lane already needs, so a working improve install has it. A tile-toolbar 🔍 button and a lightbox button run SeedVR2 **in place** (2x, result replaces the tile, original kept for undo). Requested by SurpassHR (GitHub #32).
...
- **🎭↔ Swap a tile's face with your reference photo, in one click — on any of three engines** — every tile with a current image gets a **🎭↔** button next to 🔄 Regenerate: the tile becomes the base shot, the dataset's reference photo supplies the identity, and the swap overwrites the tile in place. **Klein** (the default) repaints the head with a swap LoRA — one graph, fast, and it needs that LoRA on disk. **MiniMax H3 (new)** erases the head with a Klein pass, leaving a grey skull-shaped mannequin of where it was, and lets the video model re-render the shot around your reference — no swap LoRA, and no seam to hide, because there is no join. **MiniMax H3 (old)** is the previous graph, kept on purpose: it masks the head, sends only a crop through H3 and composites it back. **The difference that decides between them is a limit, not a preference:** the new graph re-renders the **whole frame**, so the body, clothing and background come back re-drawn rather than preserved; the old one only ever repaints inside the mask. Both H3 graphs load the whole ~40 GB H3 stack and are much slower than Klein, and the new one needs the Klein models as well. The new graph masks in the app (SAM 3 — no extra node pack to install) and ships one optional stage, an Ollama pass that reads the photo and describes how that head sits; the old one carries Klein hair removal, LaMa cleanup and a Z-Image eye/mouth detailer. The engine is a **setting, not a per-click choice** (Settings → Image engines → Klein), each engine is still **one fixed workflow rather than a configurable pipeline**, and the button only appears once the dataset has a reference photo to swap in.
...
- **🎞 A third local engine, and the first that reaches a photo through video** — **MiniMax H3** joins Klein and Krea 2 Edit: identity from one reference photo, no character LoRA, on your own GPU, NSFW allowed. It samples a short packet of frames per shot and keeps the single best one, which makes it the slowest of the three — measured on an RTX 3090, about **78 s for a new shot card and 38 s for another image of the same card**, so a batch queues each card's copies together to pay the difference once. Two limits stated up front, because both are invisible otherwise. **Nothing is auto-downloaded**: five model files (~40 GB) plus the `MinimaxH3-Image` node pack, all placed by hand — the engine card names each gap and where it goes. And **ComfyUI has to be started with `--disable-dynamic-vram`**: without it H3 loads more weights than a 24 GB card holds, nothing errors, and identical runs took anywhere from 69 s to 397 s. The app warns you when it can see that flag is missing. H3 generates dataset images only — the ✦ Edit reference lane stays Klein, Krea and the API engines.
- **🧬 A second local engine that keeps a face from one photo** — **Krea 2 Edit** joins Klein as a free, on-your-GPU way to build a dataset: give it one reference photo and it restages that person into the shots you picked — new angles, new poses, new scenes — with no character LoRA needed, because there isn't one yet. Identity, marks and piercings survive the move; a `reference grounding` dial trades likeness against how literally the shot description is followed. A front photo is all it needs, but a shot asking for a side or three-quarter view has to fight that photo's pull with words alone — so you can also upload a **Left 45° and/or Right 45° angle reference** (with crop and a one-click 180° mirror), and Krea swaps in the matching one for exactly those shots. Angle references are read by Krea 2 Edit only: Klein and Nano Banana always use the front photo, and a dataset with no angle photo uploaded behaves exactly as before. It needs two files placed by hand (a node pack and an editing LoRA) — the app checks for them and, when they are missing, names each one and where it goes rather than failing at generation time.
...
- **Reference**
  ...
  - [Run it your way](#run-it-your-way) — full local, API-only
...
- **🧑 Character** — pin an identity from one reference photo. Character LoRAs use an **activation trigger**: captions keep variable details (expression, angle, outfit) promptable while the omitted invariant — the face — binds to that token. The app can fan out a **131-shot variation catalog** (expression / angle / lighting / framing / outfit / background) so the set spans close-up to full-body without you writing a single prompt, and it runs **masked training** from auto-generated person masks. Step budget: roughly **120 steps/image** (clamped 1500–3500), and a live **12 face · 6 bust · 6 body · 1 back** composition meter rides along the whole time.
...
- **Grid actions** — resize thumbnails, zoom, crop, mirror or **↺ / ↻ rotate** individual images, then multi-select to **Keep, Reject, Undecide, clear captions, delete, or upscale via Klein or SeedVR2**. Editing a tile keeps the file's own format and doesn't re-compress it: crop the same shot ten times and the tenth is identical to the first, and a PNG or WEBP comes back pixel-for-pixel after four quarter turns (JPEG has no lossless mode, so it's re-saved at the highest practical quality). A crop is never *enlarged* beyond what you selected — it is only scaled down when a side exceeds the import resolution (1024 px by default, adjustable in Settings, up to keeping the original) — so cropping far into a photo produces a genuinely small tile, and the composition meter flags it as **⚠ Under training resolution** rather than letting it pass. Images edited before this shipped keep the pixels they already have; nothing is reprocessed retroactively. Improvements run sequentially as separate candidates and leave every source untouched: the default pass runs a small edit for sharpness/clarity, colour-matched back to your original, then SeedVR2 for the actual detail restoration; SeedVR2 alone is the fully conservative alternative, resolving detail at a higher resolution with no edit pass at all. The SeedVR2 card names the exact DiT and VAE files it will load (both lanes share the improve lane's `diffusion_models`/`vae` folders); there is no picker, because the loader only reads the folder it reads. On mouse/trackpad the per-image controls stay out of the way until hover/focus; on touch devices they remain visible. Long server-side batches (captioning, face analysis, framing, watermark) show a live progress indicator that **survives a page reload** — refresh mid-run and the button picks the batch back up instead of looking idle.
- **👤 Face-similarity scoring + auto-triage** — before an off-identity shot can poison training, **InsightFace** scores every image against your reference and badges it green (strong match) or orange (borderline), with thresholds you set in Settings. The badges you see on the grid (e.g. `0.63` green, `0.47 to review`) are exactly this: a numeric, sortable answer to *"is this even the right person?"* that your eye alone misses on shot 40. **Auto-triage** applies a chosen score threshold to currently undecided, scorable images (skipping images with no face score); during the same session you can move the threshold and re-apply it, and a later manual status change removes that row from the replay set. Scoring keeps the **best** match across your reference photo and its extra angles, which is right for a candidate but leaves one blind spot: a reference of the **wrong** person cannot lower a score — it wins that best match and *raises* every score instead. **⌖ Check**, beside the extra references, is the only pass that sees it: it compares the reference photos with each other and flags the odd one out, showing each photo's agreement figure. It **warns and never blocks**, and its threshold is deliberately forgiving so a genuine profile shot is not accused — which does mean a borderline stranger can slip past.
...
| Klein generation | ComfyUI reachable + Klein model stack |
| ✨ Upscale & improve (default `klein` engine) | ComfyUI reachable + a Krea 2 Turbo base/encoder/VAE, a Krea 2 "high quality" LoRA and a specific SeedVR2 build — none auto-downloaded, a missing one is a named error on first use; [exact files](docs/guide/settings-reference.md#seedvr2-upscaling-local) |
| SeedVR2 upscaling (standalone engine) | ComfyUI reachable + the `ComfyUI-SeedVR2_VideoUpscaler` node pack (installed from ComfyUI, not by this app — it has its own Python dependencies) + the DiT in `diffusion_models` (the same 7B Sharp int8 build the improve lane requires — no download) + the VAE in `vae` (Setup downloads it); the pipeline runs the core tiled VAE (512px tiles, always on), so no tiling pack is needed; [exact files](docs/guide/settings-reference.md#seedvr2-upscaling-local) |
...
| MiniMax H3 generation | ComfyUI reachable **and launched with `--disable-dynamic-vram`** + the `MinimaxH3-Image` node pack + five model files (Ref2VA model, Qwen3-VL 32B encoder, video VAE, audio VAE, and CLIP-ViT-H only when **Frames per shot** is above the default of 1); none auto-downloaded; [exact files](docs/guide/settings-reference.md#minimax-h3-local) |
...
| [Ollama](https://ollama.com) | Auto-captioning, framing, head-crop and watermark detection | Point **Setup** at its URL, then pull the model explicitly from LDS; native installs can use their configured URL |
...
|---|---|---|---|
| **API-only** (Gemini/ChatGPT/OpenRouter generation, import/scrape, curate, manual captions, export/backup) | none | ~2 GB | Any machine with Python 3.10+ (3.13/3.14 run the core app fine — the 3.10–3.12 window is an ML-extras constraint) |
