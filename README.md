...
| **Task Center** | Every generation, training and vision job in one live list with ComfyUI and GPU status. Submit even while ComfyUI is off: tasks pause and resume in order, and you can cancel or retry from the list. The nav badge shows in-progress tasks and flags failures until you open the page |
