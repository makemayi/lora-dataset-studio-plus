...
| **Two local engines** | **ai-toolkit** (default) or **OneTrainer**. They do not read the same settings: OneTrainer trains by **epoch**, so it gets epochs, batch size and separate text-encoder rates, and most advanced levers now reach it too (effective batch, dropout, EMA, min-SNR, save/sample cadence, rank). Alpha still follows the chosen rank, and the network type stays fixed on that lane. It also keeps its own shipped Krea 2 recipe — the app overrides nothing there until you set a value, so an empty panel is that recipe, not this app's defaults. A finished OneTrainer run auto-deploys its checkpoint into the ComfyUI LoRA folder, so it shows up on the Canvas / LoRA Studio like any other checkpoint |
