...
- **🎭↔ Swap a tile's face with your reference photo, in one click — on either of two engines** — every tile with a current image gets a **🎭↔** button next to 🔄 Regenerate: the tile becomes the base shot, the dataset's reference photo supplies the identity, and the swap overwrites the tile in place. **Klein** (the default) repaints the head with a swap LoRA — one graph, fast, and it needs that LoRA on disk. **MiniMax H3** masks the head out instead and lets the video model re-stage your reference into the hole, with no swap LoRA at all — at the price of loading the whole ~40 GB H3 stack, so it is much slower; three extra passes of it (Klein hair removal, LaMa cleanup, a Z-Image eye/mouth detailer) are switches, all off by default. The engine is a **setting, not a per-click choice** (Settings → Image engines → Klein), each engine is still **one fixed workflow rather than a configurable pipeline**, and the button only appears once the dataset has a reference photo to swap in.
